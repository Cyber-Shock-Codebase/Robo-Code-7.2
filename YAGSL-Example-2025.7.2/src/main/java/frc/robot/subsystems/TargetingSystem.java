...
import frc.robot.subsystems.swervedrive.Field.AllianceFlipUtil;
import frc.robot.subsystems.swervedrive.Field.FieldConstants.Reef;
import frc.robot.subsystems.swervedrive.Field.FieldConstants.ReefHeight;
...
public class TargetingSystem
{
  ...
  public List<Pose2d>            allianceRelativeReefBranches = null;
  ...
  public Command driveToCoralTarget(SwerveSubsystem swerveDrive)
  {
    ...
    .andThen(PositionPIDCommand.generateCommand(swerveDrive, getCoralTargetPose(), AutoConstants.kAutoAlignAdjustTimeout)));
  }

  // public Command driveToCoralTargetDefered(SwerveSubsystem swerveDrive)
  // {
  //   return DeferredCommand(() -> driveToCoralTarget(swerveDrive), Set.of(swerveDrive));
  // }

  public Command driveToCoralTargetDefer (SwerveSubsystem swerveDrive) {
  return new DeferredCommand(() -> driveToCoralTarget(swerveDrive), Set.of(swerveDrive));
}
  ...
  public Pose2d getAlgaeTargetPose()
  {
    ...
    if (targetBranch != null)
    {
      Pose2d startingPose = AllianceFlipUtil.apply(Reef.branchPositions.get(rightBranchOrdinal(targetBranch))
